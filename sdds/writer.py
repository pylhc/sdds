...
from sdds.classes import (SddsFile, Column, Parameter, Definition, Array, Data, Description,
                          ENCODING, get_dtype_str, NUMTYPES_ascii )
import itertools# <<-- jzemella @ 2022-08-25
...
def write_sdds(sdds_file: SddsFile, output_path: Union[pathlib.Path, str]) -> None:
    ...
    with output_path.open("wb") as outbytes:
        ...
        if sdds_file.mode.mode == 'binary':
            _write_data(names, sdds_file, outbytes)
        elif sdds_file.mode.mode == 'ascii':
...
def _write_header(sdds_file: SddsFile, outbytes: IO[bytes]) -> List[str]:
    ...
    if sdds_file.mode.mode == 'binary':                                 # <<-- jzemella @ 2022-08-25
        outbytes.write("!# big-endian\n".encode(ENCODING))         # <<-- jzemella @ 2022-08-25
    ...
    else:                                                                # <<-- jzemella @ 2022-08-25
         outbytes.write("&data mode=ascii, &end\n".encode(ENCODING))     # <<-- jzemella @ 2022-08-25
    return names
...
def _get_row_count(sdds_file,names):
    ...
    if sdds_file.npages > 1:
        col_data=[[sdds_file[name][0], sdds_file.values[name][0]] for name in names 
                            if isinstance(sdds_file.definitions[name],Column)]
    else:
        col_data=[sdds_file[name] for name in names 
                        if isinstance(sdds_file.definitions[name],Column)]
    ...
    if sdds_file.npages > 1:
        for npage in range(sdds_file.npages): # write pages
           # write row count 
            ...
            _write_parameters(((sdds_file[name][0],sdds_file[name][1][npage]) for name in names
                               if isinstance(sdds_file.definitions[name], Parameter)),
                              outbytes)
            _write_arrays(((sdds_file[name][0],sdds_file[name][1][npage]) for name in names
                           if isinstance(sdds_file.definitions[name], Array)),
                          outbytes)
            _write_columns([[sdds_file[name][0], sdds_file.values[name][npage]] for name in names 
                            if isinstance(sdds_file.definitions[name],Column)],
                           outbytes)
    else:
        # write row count
...
def _write_parameters(param_gen: Iterable[Tuple[Parameter, Any]], outbytes: IO[bytes]):
    for param_def, value in param_gen:
        if param_def.type in ['string']: # character will to be displayed by sddsprintout but it is not throwing an error
            ...
            outbytes.write(struct.pack(get_dtype_str("char", length=len(value)), value.encode(ENCODING)))
        else:
        ...
        for i in range(len(col_vals)):
            for j in range(len(col_vals[0])):
                ...
                if  col_def.type in ['string']:
                    ...
                    outbytes.write(np.array(value, dtype=get_dtype_str(col_def.type)).tobytes())
    except:
        pass
...
###############################################################################
# write_ascii functions ; jzemella @ 2022-08-25
###############################################################################    
...
def _write_ascii_data(names: List[str], sdds_file: SddsFile, outbytes: IO[bytes])-> None:    # one may can combine write_ascii_data with write_data ...
    ...
    if sdds_file.npages > 1:
        for npage in range(sdds_file.npages): # write pages
            ...
            _write_ascii_parameters(((sdds_file[name][0],sdds_file[name][1][npage]) for name in names
                               if isinstance(sdds_file.definitions[name], Parameter)),
                              outbytes)
            _write_ascii_arrays(((sdds_file[name][0],sdds_file[name][1][npage]) for name in names
                           if isinstance(sdds_file.definitions[name], Array)),
                          outbytes)
            _write_ascii_columns([[sdds_file[name][0], sdds_file.values[name][npage]] for name in names 
                            if isinstance(sdds_file.definitions[name],Column)],outbytes)
    else:
        _write_ascii_parameters((sdds_file[name] for name in names
                               if isinstance(sdds_file.definitions[name], Parameter)),
                              outbytes)
        _write_ascii_arrays((sdds_file[name] for name in names
                           if isinstance(sdds_file.definitions[name], Array)),
                          outbytes)
        _write_ascii_columns([sdds_file[name] for name in names 
                        if isinstance(sdds_file.definitions[name],Column)],
                           outbytes)
...
def _write_ascii_arrays(array_gen: Iterable[Tuple[Array, Any]], outbytes: IO[bytes]):
    ...
    for array_def, value in array_gen:
        ...
        for idx in itertools.product(*[range(s) for s in shape]):
           val = value[idx]
           outbytes.write(str(' ' + NUMTYPES_ascii[array_def.type] % (val)).encode(ENCODING))
        tstr='\n'   
        outbytes.write(tstr.encode(ENCODING))  
...
def _write_ascii_columns(col_data, outbytes: IO[bytes]):
    ...
    try: 
        ...
        for i in range(len(col_vals)):
            for j in range(len(col_vals[0])):
                col_val=col_vals[i]
                outbytes.write(str(' ' + NUMTYPES_ascii[col_defs[j].type] % (col_val[j])).encode(ENCODING))
            tstr='\n'
            outbytes.write(tstr.encode(ENCODING))
